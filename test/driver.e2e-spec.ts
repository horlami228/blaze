...
describe('Driver (e2e)', () => {
  ...
  it('POST /api/v1/driver/update-personal-info updates driver info', async () => {
    const res = await request(app.getHttpServer())
      .post('/api/v1/driver/update-personal-info')
      .set('Authorization', `Bearer ${authToken}`)
      .send({
        phone: '1234567890',
        gender: Gender.MALE,
  ...
  it('POST /api/v1/driver/update-driver-info updates license info and photos', async () => {
    const res = await request(app.getHttpServer())
      .post('/api/v1/driver/update-driver-info')
      .set('Authorization', `Bearer ${authToken}`)
  ...
  it('GET /api/v1/driver/manufacturer and /models lists vehicle options', async () => {
    ...
    const mRes = await request(app.getHttpServer())
      .get('/api/v1/driver/manufacturer')
      .set('Authorization', `Bearer ${authToken}`)
      .expect(200);
  ...
  it('POST /api/v1/driver/add-vehicle adds vehicle and completes onboarding', async () => {
    ...
    const res = await request(app.getHttpServer())
      .post('/api/v1/driver/add-vehicle')
      .set('Authorization', `Bearer ${authToken}`)
